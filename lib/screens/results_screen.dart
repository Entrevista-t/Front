    ...
    return AppCard(
      child: Column(
        ...
        children: [
          ...
          _reportItem('Alineació amb la pregunta', '${((r.questionAlignment ?? 0) * 100).toStringAsFixed(0)}%', kAccent, Icons.track_changes_rounded,
            description: "Mesura com de relacionada està la resposta amb la pregunta formulada."),
          _reportItem('Coherència del discurs', '${((r.discourseCoherence ?? 0) * 100).toStringAsFixed(0)}%', kAccent, Icons.account_tree_rounded,
            description: "Avalua la connexió lògica i el fil conductor entre les idees exposades."),
          _reportItem('Densitat informativa', '${((r.informationDensity ?? 0) * 100).toStringAsFixed(0)}%', kAccent, Icons.density_medium_rounded,
            description: "Proporció de paraules amb contingut rellevant respecte al total."),
          _reportItem("Índex d'especificitat", '${((r.specificityIndex ?? 0) * 100).toStringAsFixed(0)}%', kAccent, Icons.precision_manufacturing_rounded,
